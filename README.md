...
# 21. 수사팀이 두고 간 산출물 회수 — 전자정보 목록 PDF/선별 zip을
#     창 안에서 찾아 복사+해시 (휴지통 $I/$R 복원·저널 삭제 추적 포함)
...
- **fail-closed 입력 검증**: 존재하지 않는 루트·입력 파일은 즉시 거부
  (종료 코드 2) — 오타가 "빈 정상 결과"를 만들지 않는다. 예상 못한 오류는
  `raidwatch: error: …` 한 줄 + 종료 1이며, 개발용으로
  `RAIDWATCH_DEBUG=1`이면 전체 traceback을 그대로 띄운다.
